...
```
cine-dijon/
├── index.html                       la page (rien d'autre à servir)
...
Pour l'activer, le fichier doit se trouver dans **`.github/workflows/` à la racine du
dépôt**, et le dossier du projet s'appeler `cine-dijon/` (sinon, ajustez les deux
chemins dans le workflow). Dans les réglages du dépôt, `Settings → Actions → General
→ Workflow permissions`, il faut cocher **Read and write permissions** pour que le
robot puisse recommiter le fichier.
